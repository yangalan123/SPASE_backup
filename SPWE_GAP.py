...
super_para = float(sys.argv[5])
super_para_gap = float(sys.argv[6])
embedding_file = None
hownet_file = None
output_file = None
def ScorerForSememe(target):
    ...
    for word,cosine in nearestwords:
        ...
        for sememe in sememes:
            res[sememe]+=cosine*pow(para_c,rank)*pow(super_para,index)
            index += super_para_gap 
        rank+=1
    reslist = []
    for sememe in res:
    ...
    for sememe,score in reslist:
        final.append(sememe)
    return final,reslist,nearestwords
...
try:
    ...
    near_filename = sys.argv[7]
    #near_file = open(near_filename,'w')
    for line in test_data:
        print("Process:%f" %(float(len(test_list)) / len(test_data)))
        test_list.append(line.strip())
        result=[]
        result,reslist,nearestwords = ScorerForSememe(line.strip())
        output_file.write(line.strip()+"\n")
        output_file.write(' '.join(result)+"\n")
        #near_file.write(' '.join([x[0] for x in nearestwords]))
        #near_file.write('\n')
